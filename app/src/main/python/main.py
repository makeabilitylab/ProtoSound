...
def personalize_model(output_arr, shape, ways, shot, device=None):
    # data, labels = batch
    # data = data.to(device, dtype=torch.float32)
    # labels = labels.to(device, dtype=torch.long)

    # PROBLEM IS HERE!!! Convert model to support_embeddings
    support_embeddings = torch.tensor(output_arr,dtype=torch.float32)
    support_embeddings = torch.reshape(support_embeddings, tuple(shape))
    print("SUPPORT EMBEDDINGS", support_embeddings)
    print("LEN IS", len(support_embeddings))
    mean_support_embeddings = support_embeddings.reshape(ways, shot, -1).mean(dim=1)
    return mean_support_embeddings

def get_query(query_file_path, device=None):
    ...
    print("LEN IS", len(query_data))
    return query_data.tolist()

def predict_query(output_arr, shape, class_prototypes, i2c):
    query_embedding = torch.tensor(output_arr, dtype=torch.float32)
    query_embedding = torch.reshape(query_embedding, tuple(shape))
...
# classes_prototypes = personalize_model(protosound_model, batch, WAYS, SHOTS, device=device)  ## KHOA: IMPORTANT: MAKE SURE THAT "classes_prototypes" AND "support_data.i2c" ARE GLOBAL VARIABLEs WHICH NEED TO CHANGE EVERY TIME TRAINING IS DONE
